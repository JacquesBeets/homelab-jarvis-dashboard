...
try {
    const columns = db.pragma('table_info(servers)');
    if (!columns.find(c => c.name === 'url')) {
...
const statements = {
    getAll: db.prepare(`
        SELECT id, name, address, port, url, type, health_endpoint as healthEndpoint,
    ...
    insert: db.prepare(`
        INSERT INTO servers (id, name, address, port, url, type, health_endpoint, server_group, enabled, sort_order)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `),

    update: db.prepare(`
        UPDATE servers
        SET name = ?, address = ?, port = ?, url = ?, type = ?, health_endpoint = ?,
...
export const serverDb = {
    ...
    update(id, updates) {
        ...
        statements.update.run(
            ...
            updates.url !== undefined ? (updates.url || null) : existing.url,
